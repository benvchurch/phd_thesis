...
\par
Let $F$ be a invertible bundle on $X$. We note that $F$ is a divisor of some linear system {\color{red} WHAT'S THE CONTENT?}. Moreover, for any rational number $\ell \in \QQ$, we allow ourselves to form the sheaf $\ell F$, extending that we consider the tensor powers $(\ell F)^{\ot m}$ for which $m$ is such that $m \ell$ is an integer.
...
finishes the proposition. This result, along with the preceeding proposition, applied to the case $F = 0$ (so that the existence of $A$ is automatic), finishes the proof of the first assertion of the main theorem.
...
where we use $c_1^2 \le c_2$. Consider the plane in $\NS(X)_{\RR}$ spanned by the images of $F$ and $K_X$, let us take an orthonormal basis formed by a vector colinear with $K_X$ and some orthogonal vector, in which the intersection form is given by the matrix
\[ \begin{pmatrix}
1 & 0
\\
0 & -1
\end{pmatrix} \]
The points $2 \ell F$ which verify the above inequalites are the interior of the hatched area (see figure). Let $C_0$ be the open cone tangent to the positive asymtotes of the hyperbola and centered at the oiring. The line bundles $F$ verifying the inequalities are those whose images in $\NS(X)_{\RR}$ are contained in $C_0$. Let $C_1$ be the dual closed cone, and $C$ the complement, which contains the cone
...
\begin{cor}[Problem of Mordel]
Let $X$ be a minimal surface of general type, $C$ a proper smooth curve and $f : X \to C$ a morphisms with connected reduced fibers. Then if the fibration $f$ is nontrivial, there is a finique number of sections.
\end{cor}
...
\par
Therefore $E^2 \le 0$ (for another proof see [1] and [14]), if there are infinitely many they are contained in the fiberes of some foliation on $X$ and then we check that $f$ must be a trivial fibration.
\end{proof}

\begin{rmk}
In characteristic $p$, M. Raynaud [16] used this procedure to construct a counter-example to the vanishing theorem. More precisely, he constructed a fibration $f : X \to C$ whose fibers are integral curve of nonzero arithmetic genus with a section $E$ such that $E^2 > 0$.
\end{rmk}

\begin{rmk}
If we do not bound the genus, the families of curves of negative self-intersection on a surface of general type can be unbounded: let $Y \to B$ be a nonisotrivial elliptic fibration having infinitely many section of negative self-intersection (see 3.1.3). There exists a cover $X \to Y$ ramified along a smooth hyperplane section which is a surface of general type. The preimages of the sections of $Y \to $ are curves such that at least one component has negative self-intersection, and there are finitely many such curves on $X$.
\end{rmk}
...
\[ \psi_G : L_{\CC} \to H^0(X, \Omega_X^1 \ot \K_X) / H^0(X, \Omega_X^1) \]
is injective.
\end{enumerate}
...
In the above, notation, for $D \in M^{\tau}$, the meromorphic $2$-form $\omega \wedge \mu$ can be locally writen as
...
\[ H^0(X, E_{k,m}^{GG} W^*) = 0 \]
for all $k, m$. Therefore, any jet tangent to $W$ living over $x \in X$ contributes to $GG_A(X,V)$ so we conclude. There is a little subtily making this work when $W$ is not a subbundle, but then we can just shrink to throw out the codimension $2$ subset where it is not a subbundle and make the same argument since sections of the jet bundles extend.
\end{proof}
...
\begin{defn}
A bounded domain $\Omega \subset \CC^n$ is called a \textit{bounded symmetric domain} if for all $x \in \Omega$ there is an involutive ($\sigma_x^2 = \id$) biolomorphism $\sigma_x : \Omega \to \Omega$ for which $x$ is an isolated fixed point.
\end{defn}
...
\begin{proof}
The Bergman metric on $\mathbb{B}^n$ is invariant under biholomorphisms and hence descends to $X$. It has negative sectional Riemannian curvature and hence $\Omega_X$ is ample by Kodiara's theorem.
\end{proof}
...
\begin{theorem}
Let $X$ be a complex projective manifold uniformiaed by a reducible bounded symmetric domain. Then $GG(X) = X$.
\end{theorem}
