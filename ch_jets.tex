...
Then consider
\[ h^0(Y, S^m \Omega_Y(\log{E}) \ot \omega_Y^{\ot (1 - m)}) \embed h^0(Y \sm E, S^m \Omega_Y \ot \omega_Y^{\ot (1 - m)}) \]
...
by Bogomlov vanishing. Furthermore,
...
by Serre duality. Bogomolob vanishing shows that
...
\begin{rmk}
In general, this grows like $\tfrac{1}{2} m^2 K_S^2 - a_1 (2m)^2 / 4 + O(m)$ and therefore we need $c_1^2 > 2 a_1$ where $a_1$ os the number of $A_1$-singularities and also we only have $A_1$-singularities.
\end{rmk}
...
\begin{enumerate}
\item $\ell(q/n)$ is the length of the coninued fraction
...
\begin{rmk}
$I(X)$ is the index of $K_X$ menaing the smallest integer such that $I(X) K_X$ is integral. There are $\ell(X)$ exceptional curves of $S \to X$ and they are independent in $\Pic{S}$
\end{rmk}
