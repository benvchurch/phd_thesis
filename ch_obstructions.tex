...
$\pi_1(X) = \pi_1(Y) = \pi_1(Y^{\circ}) /\left<\left< \mathrm{tors} \right>\right>$ where $\left<\left< \mathrm{tors} \right>\right>$ denotes the normal subgroup generated by elements of finite order. 
\end{lemma}
...
\todo{need to square with previous stable base locus claim (is it right for elliptic curves, no bigness assumption).}
...
\end{align*}
\todo{ILLUSTRATE WITH A TIKZ PICTURE}
...
\[ g(C) = 1 + \frac{\A A_7}{2} \left[ -2 + 3 \cdot \left( 1 - \frac{1}{5} \right) \right] = 505. \]
