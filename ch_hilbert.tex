...
% REVIEW: Added definitions of modular forms, ordinary locus, and Hasse invariants with citations to Goren's lectures. Removed three \todo markers.
\todo{FIX THIS}
{\color{red}
...
\end{equation}
induced by the logarithmic Gauss-Manin connection on relative de Rham cohomology of $\cA^{\tor} \to \ol{\cM}$. 
...
which we denote by $\ol{Y_U}^{\Sigma}$ or $X_U^{\tor}$ to suppress the dependence on $\Sigma$.
\todo{
This quotient is smooth: for sufficiently small $U$ (i.e.\ $N \ge 3$), the group $G_{U,N}$ acts freely on $\ol{Y^{\cc}(N)}^{\Sigma}$ by the Diamond Lemma \cite[Lemma~2.4.1]{diamond_shurman:first_course}, and the cone decomposition $\Sigma$ was chosen to be $G_{U,N}$-invariant, so the quotient inherits smoothness.}
